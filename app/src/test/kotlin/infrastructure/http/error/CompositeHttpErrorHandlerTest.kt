...
class CompositeHttpErrorHandlerTest :
    DescribeSpec({
            ...
            it("uses the first matching handler when multiple handlers match the same type") {
                ...
                val firstHandler =
                    object : HttpErrorHandler {
                        override fun handle(t: Throwable): HttpResponse = HttpResponseFactory.error(HttpStatus.BAD_REQUEST_400, "first")
                    }
                val secondHandler =
                    object : HttpErrorHandler {
                        override fun handle(t: Throwable): HttpResponse = HttpResponseFactory.error(HttpStatus.SERVER_ERROR_500, "second")
                    }
                val handler =
                    CompositeHttpErrorHandler(
                        handlers =
